...
OBJ_FILES = $(patsubst $(srcdir)%, $(blddir)%, $(patsubst %.c, %.o, $(SOURCE_FILES)))
OBJ_FILES_C = $(filter-out $(blddir)/MQTTAsync.o $(blddir)/MQTTVersion.o $(blddir)/SSLSocket.o, $(OBJ_FILES))
OBJ_FILES_CS = $(filter-out $(blddir)/MQTTAsync.o $(blddir)/MQTTVersion.o, $(OBJ_FILES))
OBJ_FILES_A = $(filter-out $(blddir)/MQTTClient.o $(blddir)/MQTTVersion.o $(blddir)/SSLSocket.o, $(OBJ_FILES))
OBJ_FILES_AS = $(filter-out $(blddir)/MQTTClient.o $(blddir)/MQTTVersion.o, $(OBJ_FILES))
...
MQTTLIB_C_SHARED_TARGET = ${blddir}/lib${MQTTLIB_C}.so.${VERSION}
MQTTLIB_CS_SHARED_TARGET = ${blddir}/lib${MQTTLIB_CS}.so.${VERSION}
MQTTLIB_A_SHARED_TARGET = ${blddir}/lib${MQTTLIB_A}.so.${VERSION}
MQTTLIB_AS_SHARED_TARGET = ${blddir}/lib${MQTTLIB_AS}.so.${VERSION}
MQTTLIB_C_STATIC_TARGET = ${blddir}/lib${MQTTLIB_C}.a
MQTTLIB_CS_STATIC_TARGET = ${blddir}/lib${MQTTLIB_CS}.a
MQTTLIB_A_STATIC_TARGET = ${blddir}/lib${MQTTLIB_A}.a
MQTTLIB_AS_STATIC_TARGET = ${blddir}/lib${MQTTLIB_AS}.a
...
build: | mkdir ${MQTTLIB_C_SHARED_TARGET} ${MQTTLIB_CS_SHARED_TARGET} ${MQTTLIB_A_SHARED_TARGET} ${MQTTLIB_AS_SHARED_TARGET} \
	${MQTTLIB_C_STATIC_TARGET} ${MQTTLIB_CS_STATIC_TARGET} ${MQTTLIB_A_STATIC_TARGET} ${MQTTLIB_AS_STATIC_TARGET} \
...
${SYNC_TESTS}: ${blddir}/test/%: ${srcdir}/../test/%.c $(MQTTLIB_C_SHARED_TARGET)
	${CC} -g -o $@ $< -l${MQTTLIB_C} ${FLAGS_EXE}

${SYNC_SSL_TESTS}: ${blddir}/test/%: ${srcdir}/../test/%.c $(MQTTLIB_CS_SHARED_TARGET)
	${CC} -g -o $@ $< -l${MQTTLIB_CS} ${FLAGS_EXES}

${ASYNC_TESTS}: ${blddir}/test/%: ${srcdir}/../test/%.c $(MQTTLIB_CS_SHARED_TARGET)
	${CC} -g -o $@ $< -l${MQTTLIB_A} ${FLAGS_EXE}

${ASYNC_SSL_TESTS}: ${blddir}/test/%: ${srcdir}/../test/%.c $(MQTTLIB_CS_SHARED_TARGET) $(MQTTLIB_AS_SHARED_TARGET)
	${CC} -g -o $@ $< -l${MQTTLIB_AS} ${FLAGS_EXES}

${SYNC_SAMPLES}: ${blddir}/samples/%: ${srcdir}/samples/%.c $(MQTTLIB_C_SHARED_TARGET)
	${CC} -o $@ $< -l${MQTTLIB_C} ${FLAGS_EXE}

${ASYNC_SAMPLES}: ${blddir}/samples/%: ${srcdir}/samples/%.c $(MQTTLIB_A_SHARED_TARGET)
	${CC} -o $@ $< -l${MQTTLIB_A} ${FLAGS_EXE}

${MQTTLIB_C_STATIC_TARGET}: ${OBJ_FILES_C} ${HEADERS_C}
	$(AR) cru $@ ${OBJ_FILES_C}

${MQTTLIB_CS_STATIC_TARGET}: ${OBJ_FILES_CS} ${HEADERS_C}
	$(AR) cru $@ ${OBJ_FILES_CS}

${MQTTLIB_A_STATIC_TARGET}: ${OBJ_FILES_A} ${HEADERS_A}
	$(AR) cru $@ ${OBJ_FILES_A}

${MQTTLIB_AS_STATIC_TARGET}: ${OBJ_FILES_AS} ${HEADERS_A}
	$(AR) cru $@ ${OBJ_FILES_AS}

${MQTTLIB_C_SHARED_TARGET}: ${SOURCE_FILES_C} ${HEADERS_C}
...
${MQTTLIB_CS_SHARED_TARGET}: ${SOURCE_FILES_CS} ${HEADERS_C}
	$(SED_COMMAND) $(srcdir)/MQTTClient.c
...
${MQTTLIB_A_SHARED_TARGET}: ${SOURCE_FILES_A} ${HEADERS_A}
...
${MQTTLIB_AS_SHARED_TARGET}: ${SOURCE_FILES_AS} ${HEADERS_A}
	$(SED_COMMAND) $(srcdir)/MQTTAsync.c 
...
${MQTTVERSION_TARGET}: $(srcdir)/MQTTVersion.c $(srcdir)/MQTTAsync.h ${MQTTLIB_A_SHARED_TARGET} $(MQTTLIB_CS_SHARED_TARGET)
...
install: build
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_C_STATIC_TARGET} $(DESTDIR)${libdir}
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_CS_STATIC_TARGET} $(DESTDIR)${libdir}
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_A_STATIC_TARGET} $(DESTDIR)${libdir}
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_AS_STATIC_TARGET} $(DESTDIR)${libdir}
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_C_SHARED_TARGET} $(DESTDIR)${libdir}
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_CS_SHARED_TARGET} $(DESTDIR)${libdir}
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_A_SHARED_TARGET} $(DESTDIR)${libdir}
	$(INSTALL_DATA) ${INSTALL_OPTS} ${MQTTLIB_AS_SHARED_TARGET} $(DESTDIR)${libdir}
...
uninstall:
	rm $(DESTDIR)${libdir}/lib$(MQTTLIB_C).a
	rm $(DESTDIR)${libdir}/lib$(MQTTLIB_CS).a
	rm $(DESTDIR)${libdir}/lib$(MQTTLIB_A).a
	rm $(DESTDIR)${libdir}/lib$(MQTTLIB_AS).a
